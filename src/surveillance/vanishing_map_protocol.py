...
class Drone(IProtocol):
    ...
    def new_mobility_command(self):
        minimum_cells = self.check_minimum_cells()                   
        logging.info(f"Node {self.provider.get_id()}. There are {len(minimum_cells)} minimum cells")

        if minimum_cells:
            new_target = float(np.random.choice(minimum_cells))
    ...
    def check_minimum_cells(self):
        #~[]
        #elf._log.info(f"Minimum cell value is {self.map[:,0].min()}")
        return np.where(self.map[:, 0] == self.map[:,0].min())[0].tolist()
    ...
    def updated_map(self, data: dict):
        ...
        #minimum_cells = self.check_minimum_cells()
        #self._log.info(f"Node {self.provider.get_id()}. There are {len(minimum_cells)} minimum cells.")
